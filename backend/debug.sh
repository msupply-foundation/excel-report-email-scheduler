...
PORT="${2:-3222}"
PLUGIN_NAME="${1:-gpx_msupply-datasource_darwin_amd64}"
...
IFS=- read -ra PLUGIN_PIDS <<< "$PLUGIN_PID"
dlv attach ${PLUGIN_PID/${PLUGIN_PIDS[0]}} --headless --listen=:${PORT} --api-version 2 --log
pkill dlv