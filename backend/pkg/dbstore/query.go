...
func (datasource *SQLiteDatasource) query(ctx context.Context, query backend.DataQuery) (response backend.DataResponse) {
	var qm queryModel
	...
	err = replaceVariables(&queryConfig, query)
	if err != nil {
		response.Error = err
		return response
	}
	...
	columns, err := fetchData(datasource, &queryConfig)
	if err != nil {
		response.Error = err
		return response
	}
	...
	log.DefaultLogger.Debug("My columns", columns)
...
func fetchData(datasource *SQLiteDatasource, queryConfig *queryConfigStruct) (columns []*sqlColumn, err error) {
