...
func (server *HttpServer) fetchReportContent(rw http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		log.DefaultLogger.Error("fetchReportContent: db.GetReportContent")
	...
	if err != nil {
		log.DefaultLogger.Error("fetchReportContent: json.NewEncoder().Encode()")
		http.Error(rw, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (server *HttpServer) deleteReportContent(rw http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		log.DefaultLogger.Error("deleteReportContent: db.DeleteReportContent()")
		http.Error(rw, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (server *HttpServer) createReportContent(rw http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		log.DefaultLogger.Error("createReportContent: json.NewEncoder().Encode()")
		http.Error(rw, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (server *HttpServer) updateReportContent(rw http.ResponseWriter, request *http.Request) {
	...
	json.NewEncoder(rw).Encode(reportContent)
	rw.WriteHeader(http.StatusOK)
}
